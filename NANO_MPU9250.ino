/* 
logger for Kris Winer's Nano MPU9250 board
by MGP from  MPU9250_MS5637_t3 Basic Example Code by Kris Winer, April 1, 2014
license: Beerware 
...
uint32_t delt_t = 0, tnow = 0, tprev = 0, sumCount = 0;  // used to control display output rate
...
void setup()
{
	...
	if (c == 0x71) 
	{
		...
		initMPU9250();
		Serial.println("Accelerometer & gyro ready"); // Initialize device for active mode read of acclerometer, gyroscope, and temperature
		Serial.println(" ");
																		// Read the WHO_AM_I register of the magnetometer, this is a good test of communication
		byte d = readByte(AK8963_ADDRESS, WHO_AM_I_AK8963);  // Read WHO_AM_I register for AK8963
		if (d==0x48) Serial.println("AK8963 magnetometer found");
		...
		Serial.println("Magnetometer scale (mG)"); 
		Serial.print(magScale[0]);  Serial.print(", ");
		Serial.print(magScale[1]);  Serial.print(", ");
		Serial.println(magScale[2]);

		Serial.println("Magnetometer ready"); // Initialize device for active mode read of magnetometer
		delay(5000); // add delay to see results before serial spew of data

		if (SerialDebug) {
			Serial.println("Magnetometer calibration adjustments: ");
			Serial.print(magCalibration[0], 2); Serial.print(", ");
			Serial.print(magCalibration[1], 2); Serial.print(", ");
	...
	// Header
	Serial.print("t, ax, ay, az, gx, gy, gz, mx, my, mz");
	//Serial.print", q0, q1, q2, q3 ");
	Serial.println("");
...
/* MadgwickQuaternionUpdate(-ax, ay, az, gx*PI / 180.0f, -gy*PI / 180.0f, -gz*PI / 180.0f, my, -mx, mz);  */
	...
	tnow = micros();
	delt_t = tnow - tprev;
	if (delt_t >= 2500) { // independent of read rate
		...
		if (newData == true) {  // On interrupt, read data
			...
			Serial.print((double)tnow/1000000.0d, 4); Serial.print(", ");
...
void initMPU9250()
{
	...
	c = c | 0x08;  // accel_fchoice_b bit [3] = 1 (4 KHz sample rate, filter off)
